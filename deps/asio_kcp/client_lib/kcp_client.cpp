...
kcp_client::kcp_client(void)
    : in_connect_stage_(false), connect_start_time_(0),
      last_send_connect_msg_time_(0), connect_succeed_(false),
      pevent_func_(NULL), event_callback_var_(NULL), udp_port_bind_(0),
      server_port_(0), udp_socket_(-1), p_kcp_(NULL)
{
...
void kcp_client::set_event_callback(
...
void kcp_client::init_kcp(kcp_conv_t conv)
{
    ...
    ikcp_nodelay(
        p_kcp_, 1, 2, 1,
        1); // 设置成1次ACK跨越直接重传, 这样反应速度会更快. 内部时钟5毫秒.
}

int kcp_client::connect_async(
    int udp_port_bind, const std::string& server_ip, const int server_port)
{
    if (udp_socket_ != -1)
...
void kcp_client::do_asio_kcp_connect(uint64_t cur_clock)
{
    if (connect_timeout(cur_clock)) {
        (*pevent_func_)(
            0, eConnectFailed, KCP_CONNECT_TIMEOUT_MSG, event_callback_var_);
...
{
    return (
        cur_clock - last_send_connect_msg_time_ >
        KCP_RESEND_CONNECT_MSG_INTERVAL);
}
...
void kcp_client::try_recv_connect_back_packet(void)
{
    ...
    const ssize_t ret_recv = recv(
        udp_socket_, recv_buf, sizeof(recv_buf),
        0); // recv timeout is 2 milliseconds. - SO_RCVTIMEO
    if (ret_recv < 0) {
        int err = errno;
        if (err == EAGAIN)
            return;
    ...
    if (ret_recv > 0 &&
        ...
        kcp_conv_t conv =
            asio_kcp::grab_conv_from_send_back_conv_packet(recv_buf, ret_recv);

        /*
        std::cerr << "connect succeed in " << iclock64() - connect_start_time_
        << " milliseconds"
            << " conv:" << conv
            << std::endl;
        */
        ...
        (*pevent_func_)(
            p_kcp_->conv, eConnect, "connect succeed", event_callback_var_);
    }
}
/*
int kcp_client::do_asio_kcp_connect_old(void)
{
    ...
    while (true)
    {
        ...
        const ssize_t send_ret = send(udp_socket_, connect_msg.c_str(),
connect_msg.size(), 0); if (send_ret < 0)
        {
...
errno << " " << strerror(errno) << std::endl;
        ...
        for (int j = 0; j < 250; j++)
        {
            const ssize_t ret_recv = recv(udp_socket_, recv_buf,
sizeof(recv_buf), 0); // recv timeout is 2 milliseconds. - SO_RCVTIMEO if
(ret_recv < 0)
            {
                int err = errno;
                if (err == EAGAIN)
                    continue;
                std::cerr << "do_asio_kcp_connect recv error return with errno:
" << err << " " << strerror(err) << std::endl;
            }
            if (ret_recv > 0 && asio_kcp::is_send_back_conv_packet(recv_buf,
ret_recv))
            {
                // connect ok.
                std::cerr << "connect succeed in " << ::time(NULL) - begin_time
<< " seconds" << std::endl;
...
int kcp_client::init_udp_connect(void)
{
    ...
    /*{
        ...
        int ret = setsockopt(udp_socket_, SOL_SOCKET, SO_RCVTIMEO, &recv_timeo,
    sizeof(recv_timeo)); if (ret < 0)
        {
...
{
    char recv_buf[MAX_MSG_SIZE * 2] =
        ""; // udp packet will not twice bigger than kcp msg size.
...
void kcp_client::handle_udp_packet(const std::string& udp_packet)
{
    if (is_disconnect_packet(udp_packet.c_str(), udp_packet.size())) {
        if (pevent_func_ != NULL) {
            ...
            (*pevent_func_)(
                p_kcp_->conv, eDisconnect, msg, event_callback_var_);
        }
        return;
    }
