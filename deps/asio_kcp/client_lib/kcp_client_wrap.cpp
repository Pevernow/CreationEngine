...
kcp_client_wrap::kcp_client_wrap(void)
    : connect_result_(1), pevent_func_(NULL), event_func_var_(NULL),
      workthread_(0), workthread_want_stop_(false), workthread_stopped_(false),
      workthread_start_(false), kcp_last_update_clock_(0)
{
...
void kcp_client_wrap::handle_client_event_callback(
    kcp_conv_t conv, eEventType event_type, const std::string& msg)
{
    std::cout << "kcp_client_wrap::handle_client_event_callback event_type: "
              << event_type << std::endl;
    switch (event_type) {
        case eConnect:
            connect_result_ = 0;
...
int kcp_client_wrap::connect_async(
    int udp_port_bind, const std::string& server_ip, const int server_port)
{
    int ret_connect_async =
        kcp_client_.connect_async(udp_port_bind, server_ip, server_port);
...
void kcp_client_wrap::start_workthread(void)
{
    ...
    int ret = pthread_create(
        &workthread_, NULL, &kcp_client_wrap::workthread_loop, (void*)this);
    if (ret != 0) {
        std::cerr << "start_workthread pthread_create error: " << ret
                  << std::endl;
        return;
    }
