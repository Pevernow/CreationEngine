...
connection::shared_ptr connection_container::find_by_conv(
    const kcp_conv_t& conv)
{
...
connection::shared_ptr connection_container::add_new_connection(
    std::weak_ptr<connection_manager> manager_ptr, const kcp_conv_t& conv,
    const udp::endpoint& udp_sender_endpoint)
{
    connection::shared_ptr ptr =
        connection::create(manager_ptr, conv, udp_sender_endpoint);
