...
connection::connection(const std::weak_ptr<connection_manager>& manager_ptr)
    : connection_manager_weak_ptr_(manager_ptr), conv_(0), p_kcp_(NULL),
...
connection::shared_ptr connection::create(
    const std::weak_ptr<connection_manager>& manager_ptr,
...
void connection::set_udp_remote_endpoint(
...
void connection::send_udp_package(const char* buf, int len)
{
    if (auto ptr = connection_manager_weak_ptr_.lock()) {
        ...
        std::cout << "udp_send:" << udp_remote_endpoint_.address().to_string()
                  << ":" << udp_remote_endpoint_.port() << " conv:" << conv_
                  << " size:" << len << "\n"
...
void connection::input(
    char* udp_data, size_t bytes_recvd,
    const udp::endpoint& udp_remote_endpoint)
{
...

    {
        ...
        if (kcp_recvd_bytes <= 0) {
            ...
            std::cout << "\n"
                      ...
                      << " lag_time:"
                      << get_cur_clock() - last_packet_recv_time_
                      << " kcp recv: " << kcp_recvd_bytes << std::endl
                      << Essential::ToHexDumpText(package, 32) << std::endl;
            */
            if (auto ptr = connection_manager_weak_ptr_.lock()) {
                ptr->call_event_callback_func(
                    conv_, eRcvMsg, std::make_shared<std::string>(package));
            }
        }
    }
}
