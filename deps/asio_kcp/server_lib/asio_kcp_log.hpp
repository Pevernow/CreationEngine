...
// using muduo for kcp packet log. Because I want to split the logic log and the
// package log.
...
// {
#define AK_LOG                                                                 \
    LOG // LOG(FATAL) << "This message is FATAL. After log flush -> Abort()";
#define AK_LOGF                                                                \
    LOGF // LOGF(WARNING, "Printf-style syntax is also %s", "available");
...
//#define AK_CHECKF CHECKF    // CHECKF(foo(), "if 'false == foo()' then the %s
//is broken: FATAL), "contract");
// }
