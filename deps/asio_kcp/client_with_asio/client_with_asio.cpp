...
std::string get_cur_time_str()
{
    ...
    osstrm << tmp_ptr->tm_hour << ":" << tmp_ptr->tm_min << "."
           << tmp_ptr->tm_sec;
    return osstrm.str();
}
...
uint64_t get_time_from_msg(const std::string& msg)
{
    ...
    const std::string& time_str =
        msg.substr(0 + sizeof(CLOCK_START_STR) - 1, pos);
    // std::cout << "time_str: " << time_str << std::endl;
    return std::atoll(time_str.c_str());
}

std::unordered_map<uint64_t /*package_send_time*/, size_t /*send_counter*/>
    g_package_send_counter;
uint64_t search_time_from_kcp_str(const std::string& kcp_str)
{
    // std::cout << "udp send: " << kcp_str.size() << std::endl <<
    // Essential::ToHexDumpText(kcp_str, 32) << std::endl;
    ...
    uint64_t ret =
        std::atoll(&kcp_str[start_iter] + sizeof(CLOCK_START_STR) - 1);
...
client_with_asio::client_with_asio(
    asio::io_service& io_service, int udp_port_bind,
    const std::string& server_ip, const int server_port,
    const size_t test_str_size)
    : stopped_(false), test_str_size_(test_str_size), client_timer_(io_service),
...
void client_with_asio::print_recv_log(const std::string& msg)
{
    ...
    // std::cout << interval << ":" << send_time << ":" <<
    // g_package_send_counter[send_time] << "\t";
    ...
    if (static_good_recv_count % 10 == 0) {
        ...
        std::cout << "max:"
                  << *std::max_element(
                         recv_package_interval10_.begin(),
                         recv_package_interval10_.end())
                  << " avrg10:" << average10;
        recv_package_interval10_.clear();
    }

    if (cur_time - static_last_refresh_time > 10 * 1000 &&
        static_good_recv_count % 100 == 30 && static_good_recv_count != 30) {
        std::cout << " "
                  << static_cast<double>(
                         static_recved_bytes * 10 /
                         (cur_time - static_last_refresh_time)) /
                         10
                  << "KB/s(in)";
    ...
    if (static_good_recv_count % 100 == 0 && static_good_recv_count != 0) {
        ...
        std::cout << " max100:"
                  << *std::max_element(
                         recv_package_interval100_.begin(),
                         recv_package_interval100_.end())
                  << " avrg100:" << average100 << " avrgall:" << average_total;
        recv_package_interval100_.clear();
    }

    if (static_good_recv_count % 100 == 10 && static_good_recv_count != 10) {
        std::cout << " ext_packet_snd:"
                  << (g_count_send_udp_packet * 100 / g_count_send_kcp_packet)
                  << "%"
                  << " ext_size_snd:"
                  << g_count_send_udp_size * 100 / g_count_send_kcp_size << "%";
    }
...
void client_with_asio::hook_client_timer(void)
{
    if (stopped_)
        return;
    client_timer_.expires_from_now(
        boost::posix_time::milliseconds(5)); // 1 millisecond will be better
    client_timer_.async_wait(
        std::bind(&client_with_asio::handle_client_time, this));
}
...
void client_with_asio::hook_timer_send_msg(void)
{
    if (stopped_)
        return;
    client_timer_send_msg_.expires_from_now(
        boost::posix_time::milliseconds(SEND_TEST_MSG_INTERVAL));
    client_timer_send_msg_.async_wait(
        std::bind(&client_with_asio::handle_timer_send_msg, this));
}
...
void client_with_asio::client_event_callback_func(
    kcp_conv_t conv, eEventType event_type, const std::string& msg, void* var)
{
    ((client_with_asio*)var)
        ->handle_client_event_callback(conv, event_type, msg);
}

void client_with_asio::handle_client_event_callback(
    kcp_conv_t conv, eEventType event_type, const std::string& msg)
{
    switch (event_type) {
        case eConnect:
        ...
        case eConnectFailed:
            ...
            break;
        case eRcvMsg:
            std::cout << "recv eRcvMsg with conv:" << conv
                      << "  msg_count: " << msg.size() << std::endl;
            break;
        case eDisconnect:
            std::cout << "disconnect with conv:" << conv << " msg: " << msg
                      << std::endl;
            stopped_ = true; // you can add asio::io_service::work to prevent
                             // program quit
            break;
        default:; // do nothing
    }
